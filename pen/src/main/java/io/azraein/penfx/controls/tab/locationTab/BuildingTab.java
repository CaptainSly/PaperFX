...
import io.azraein.inkfx.system.locations.buildings.BuildingType;
...
public class BuildingTab extends PaperEditorTab {
    ...
    public BuildingTab(PenFX penFX) {
        super(penFX);
        ...
        dbBuildingView.getItems().addAll(penFX.getObservableBuildingRegistry().values());
            ...
            public void updateItem(Building item, boolean empty) {
                ...
                if (item != null) {
                    ...
                    removeBuildingItem.setOnAction(event -> {
                        Database pluginDb = penFX.currentPluginProperty().get().getPluginDatabase();
                        if (pluginDb.getBuildingRegistry().containsKey(item.getBuildingId())) {
                            pluginDb.getBuildingRegistry().remove(item.getBuildingId());
                            penFX.getObservableBuildingRegistry().remove(item.getBuildingId());
                        }
        ...
        dbBuildingView.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (newValue != null) {
                ...
                for (String actionId : newValue.getBuildingActionIds()) {
                    Action action = penFX.getObservableActionRegistry().get(actionId);
        ...
        penFX.getObservableBuildingRegistry().addListener((MapChangeListener<String, Building>) change -> {
            ...
            if (change.wasAdded()) {
                if (penFX.currentPluginProperty().get().getPluginDatabase().getBuildingRegistry()
        ...
        buildingActionSelector.getItems().addAll(penFX.getObservableActionRegistry().values());
        ...
        buildingNpcSelectorBox.getItems().addAll(penFX.getObservableNpcRegistry().values());
        ...
        saveBuildingBtn.setOnAction(event -> {
            ...
            penFX.currentPluginProperty().get().getPluginDatabase().addBuilding(building);
            penFX.getObservableBuildingRegistry().put(building.getBuildingId(), building);
        });
