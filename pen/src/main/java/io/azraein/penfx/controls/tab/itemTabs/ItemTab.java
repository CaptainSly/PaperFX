package io.azraein.penfx.controls.tab.itemTabs;
...
import io.azraein.inkfx.system.inventory.items.ItemType;
...
public class ItemTab extends PaperEditorTab {
    ...
    public ItemTab(PenFX penFX) {
        ...
        chooseItemImageButton.setOnAction(event -> {
            ...
            File image = fc.showOpenDialog(penFX.getPrimaryStage().getOwner());
            if (image.exists()) {
        ...
        chooseItemScriptButton.setOnAction(event -> {
            ...
            File script = fc.showOpenDialog(penFX.getPrimaryStage().getOwner());
            if (script.exists()) {
        ...
        saveItemButton.setOnAction(event -> {
            ...
            penFX.currentPluginProperty().get().getPluginDatabase().addItem(item);
            penFX.getObservableItemRegistry().put(item.getItemId(), item);
        });
                    ...
                    removeItem.setOnAction(event -> {
                        ...
                        if (penFX.currentPluginProperty().get() != null) {
                            ...
                            if (curPluginDb.getItemRegistry().containsKey(_item.getItemId())) {
                                ...
                                penFX.getObservableItemRegistry().remove(_item.getItemId());
                            }
                        }
                    });
        ...
        dbItemLV.getItems().addAll(penFX.getObservableItemRegistry().values());

        // #endregion

        // #region Listener setup

        penFX.getObservableItemRegistry().addListener((MapChangeListener<String, Item>) change -> {

            if (change.wasAdded()) {
                if (penFX.currentPluginProperty().get().getPluginDatabase().getItemRegistry()
