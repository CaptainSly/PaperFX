...
public class ActorClassTab extends PaperEditorTab {
	...
	public ActorClassTab(PenFX penFX) {
		super(penFX);
		...
		actorClassList.getItems().addAll(penFX.getObservableActorClassRegistry().values());
		penFX.getObservableActorClassRegistry().addListener((Change<? extends String, ? extends ActorClass> change) -> {
			if (change.wasAdded()) {
					...
					removeClass.setOnAction(event -> {
						...
						if (penFX.currentPluginProperty().get() != null) {
							...
							if (curPluginDb.getActorClassRegistry().containsKey(ac.getActorClassId())) {
								...
								penFX.getObservableActorClassRegistry().remove(ac.getActorClassId());
							}
						}
		...
		saveActorClassBtn.setOnAction(event -> {
			...
			if (penFX.getObservableActorClassRegistry().containsKey(actorClassId)) {
				actorClass = penFX.getObservableActorClassRegistry().get(actorClassId);
			}
			...
			penFX.currentPluginProperty().get().getPluginDatabase().addActorClass(actorClass);
			penFX.getObservableActorClassRegistry().put(actorClass.getActorClassId(), actorClass);
		});
